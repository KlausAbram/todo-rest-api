...
func (r *TodoItemPostgres) Create(listId int, input todoapi.TodoItem) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todoapi.TodoItem, error) {
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (todoapi.TodoItem, error) {
	var item todoapi.TodoItem
...
func (r *TodoItemPostgres) Delete(userId, itemId int) error {
	query := fmt.Sprintf("DELETE FROM %s ti USING %s li, %s ul WHERE ti.id = li.item_id AND ul.list_id = li.list_id AND ul.user_id=$1 AND ti.id=$2",
		todoItemTable, listsItemsTable, usersListsTable)
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todoapi.UpdateItemInput) error {
